...
public class nameComparator implements Comparator<Student> {
	...
	public int compare(Student o1, Student o2) {
		...
		if (arr1[arr1.length - 1].compareTo(arr2[arr2.length - 1]) < 0) {
			...
			if (arr1[arr1.length - 2].compareTo(arr2[arr2.length - 2]) <0) {
				return -1;
			} else if (arr1[arr1.length - 2].compareTo(arr2[arr2.length - 2]) > 0) {
				return 1;
			} else if (arr1[arr1.length - 2].compareTo(arr2[arr2.length - 2]) == 0) {
				if (arr1[arr1.length - 3].compareTo(arr2[arr2.length - 3]) < 0) {
					return -1;
				} else if (arr1[arr1.length - 3].compareTo(arr2[arr2.length - 3]) > 0) {
					return 1;
				} else {
					return 0;
				}
